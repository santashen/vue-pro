...
# Flask Heat Calculator 启动脚本

echo "正在启动 Flask Heat Calculator 应用..."
...
if [[ -f "$PYTHON_PATH" ]]; then
    ...
    /data/miniconda3/envs/light/bin/gunicorn wsgi:app --bind 127.0.0.1:5003
else
    echo "警告：conda 环境不存在，尝试使用系统 Python"
    python3 -m gunicorn wsgi:app --bind 127.0.0.1:5003
fi 